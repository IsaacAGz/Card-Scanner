...
1. Place your Scryfall bulk data JSON (unique-artwork...json) in the root directory.

2. Run auto_card_downloader.py to populate the reference_cards folder.

3. Run create_index.py. This will generate mtg_cards.index and mtg_cards.db.

**Phase 2: Running the Scanner**
  1. Ensure your phone is streaming an MJPEG feed (default URL in scripts is http://192.168.100.8:8080/shot.jpg).

  2. Run python card_scanner_embedded.py.

  3. Hold a card in front of the camera. The system will draw a green border and display the identified card name.

**Phase 3: Batch Image Processing**

  1. Place images you want to analyze in the images_to_scan folder.

  2. Run python card_scanner_images_embedded.py.

  3. The script will output new images with bounding boxes and names labeled (e.g., analyzed_photo.jpg).
